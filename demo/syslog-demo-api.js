...
(function () {
    ...
    add(5, 'edge-fw', '10.20.0.1', 'syslog', 'filterlog', 16, sev(6, 4), (k) =>
        `${k % 2 ? 'pass' : 'block'},in,igb0,tcp,10.31.${k % 254}.${(k >> 3) % 254}:${1024 + (k % 50000)},10.20.10.11:443`);
    add(2, 'edge-fw', '10.20.0.1', 'syslog', 'openvpn', 1, sev(5, 5), (k) =>
        `user 'remote${1 + (k % 9)}' authenticated, session established from 172.16.${k % 254}.${(k >> 2) % 254}`);
    add(1, 'edge-fw', '10.20.0.1', 'syslog', 'dhcpd', 1, sev(6, 6), (k) =>
        `DHCPACK on 10.20.20.${100 + (k % 120)} to aa:bb:cc:${(k % 256).toString(16).padStart(2, '0')}:1f:0${k % 10} via lan0`);
    add(3, 'core-sw', '10.20.0.2', 'syslog', 'link', 23,
        ...
        (k) => k % 23 === 0 ? `%LINEPROTO-5-UPDOWN: Line protocol on Interface GigabitEthernet1/0/${1 + (k % 24)}, changed state to ${k % 2 ? 'up' : 'down'}`
                            : `%SEC_LOGIN-5-LOGIN_SUCCESS: Login Success [user: netops] [Source: 10.20.10.15] [localport: 22]`);
    add(1, 'core-sw', '10.20.0.2', 'trap', 'snmptrap', 23, () => 5, (k) =>
        `linkUp trap: ifIndex ${1 + (k % 24)}, ifDescr GigabitEthernet1/0/${1 + (k % 24)}, ifOperStatus up(1)`);
    add(3, 'intranet-01', '10.20.10.11', 'syslog', 'nginx', 1, sev(6, 4), (k) =>
        `10.31.${(k >> 1) % 254}.${k % 254} "GET /${['', 'api/orders', 'assets/app.js', 'login', 'api/health'][k % 5]} HTTP/1.1" ${k % 29 === 0 ? 502 : 200} ${180 + (k % 42000)}`);
    add(1, 'intranet-01', '10.20.10.11', 'syslog', 'sshd', 4, sev(6, 4), (k) =>
        k % 11 === 0 ? `Failed password for invalid user admin from 10.31.7.${k % 254} port ${40000 + (k % 20000)} ssh2`
                     : `Accepted publickey for deploy from 10.20.10.15 port ${40000 + (k % 20000)} ssh2`);
    add(2, 'nas-01', '10.20.10.20', 'syslog', 'smbd', 1, sev(6, 5), (k) =>
        `connection to service backup from intranet-01 (10.20.10.11) ${k % 2 ? 'opened' : 'closed'}`);
    add(1, 'nas-01', '10.20.10.20', 'syslog', 'zfs', 1, sev(5, 5), (k) =>
        `zpool volume1 scrub in progress: ${(k % 97) + 1}% done, 0 errors`);
    add(2, 'vhost-cluster', '10.20.10.30', 'syslog', 'pvedaemon', 1, sev(6, 5), (k) =>
        `<root@pam> ${['starting task vzdump', 'end task vzdump OK', 'VM 10${k % 9} qmp command success', 'migration of VM 10' + (k % 9) + ' finished'][k % 4]}`);
    add(1, 'mon-01', '10.20.10.15', 'syslog', 'cron', 9, () => 6, (k) =>
        `(root) CMD (/usr/local/bin/feed-check.sh${k % 2 ? ' --quiet' : ''})`);
    add(1, 'ap-hq-01', '10.20.20.10', 'syslog', 'hostapd', 1, sev(6, 5), (k) =>
        `wlan0: STA 3a:7f:12:9d:2${k % 10}:1b IEEE 802.11: ${k % 2 ? 'associated' : 'disassociated'}`);
    add(1, 'ups-01', '10.20.10.44', 'trap', 'snmptrap', 23, () => 5, (k) =>
    ...
    window.fetch = function (url, opts) {
        ...
        if (path === '/api/stats') {
            return reply({
                ...
                dbBytes: 241563648,
                topSources: [
                    { sourceIp: '10.20.0.1', n: 168044 }, { sourceIp: '10.20.10.11', n: 92310 },
                    { sourceIp: '10.20.0.2', n: 74921 }, { sourceIp: '10.20.10.30', n: 55870 },
                    { sourceIp: '10.20.10.20', n: 48112 }, { sourceIp: '10.20.20.10', n: 24788 },
                    { sourceIp: '10.20.10.15', n: 20416 }, { sourceIp: '10.20.10.44', n: 8241 }
                ],
